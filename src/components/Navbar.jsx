  ...
  return (
    <>
      <motion.header
        ...
        className="fixed left-1/2 top-4 z-50 hidden w-[min(96vw,84rem)] -translate-x-1/2 md:block"
      >
        <div className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-4 rounded-[1.9rem] border border-white/10 bg-[rgba(15,10,31,0.82)] px-5 py-3 shadow-[0_24px_70px_rgba(0,0,0,0.34)] backdrop-blur-2xl">
          <a
            href="#home"
          ...
          <nav className="flex min-w-0 items-center justify-center gap-2 overflow-x-auto justify-self-center">
            {navLinks.map((link) => {
      ...
      <motion.button
        ...
        className="fixed right-4 top-4 z-[60] flex items-center gap-2 rounded-full border border-white/10 bg-[rgba(15,10,31,0.82)] px-4 py-3 text-sm font-semibold text-white shadow-[0_20px_60px_rgba(0,0,0,0.32)] backdrop-blur-2xl transition hover:-translate-y-0.5 hover:border-roseglow-300/40 hover:bg-[rgba(22,12,42,0.94)]"
        aria-expanded={open}
        ...
        {open && (
          <>
            ...
            <motion.aside
              ...
              transition={{ duration: 0.25, ease: 'easeOut' }}
              className="fixed right-4 top-20 z-50 w-[min(90vw,20rem)]"
            >
                    ...
                    {navLinks.map((link) => {
                      ...
                      return (
                        <a
                          ...
                          href={link.href}
                          onClick={() => setOpen(false)}
                          className={`group flex items-center gap-3 rounded-[1.25rem] border px-4 py-3 text-sm transition duration-300 hover:-translate-y-0.5 ${activeState ? 'border-roseglow-300/50 bg-white/10 text-white' : 'border-white/10 bg-white/[0.04] text-white/72 hover:border-white/20 hover:bg-white/[0.08] hover:text-white'}`}
                        >
                          <Icon className={`h-4 w-4 ${activeState ? 'text-roseglow-200' : 'text-white/45 group-hover:text-white'}`} />
                          <span>{link.label}</span>
                          <Circle className={`ml-auto h-2 w-2 ${activeState ? 'fill-roseglow-300 text-roseglow-300' : 'text-white/20'}`} />
                        </a>
                      )
                    })}
                  </div>
